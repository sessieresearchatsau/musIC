...
def float_to_fraction(value, max_denominator=1000):
    ...
    The 'max_denominator' argument limits fraction complexity.
    """
...
def generate_minimal_output(track_data_list, output_path):
    ...
    where 'start' and 'duration' are shown as fractions (e.g. "1/2"),
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for i, note_events in enumerate(track_data_list):
            ...
            for ev in note_events:
                ...
                # Convert to fraction strings
                start_fraction = float_to_fraction(start_sec)
                duration_fraction = float_to_fraction(duration_sec)

                # {start,duration,note}, no spaces
                sets_list.append(
                    f"{{{start_fraction},{duration_fraction},{note_num}}}"
                )
